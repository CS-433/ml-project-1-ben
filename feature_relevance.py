...
def evaluate_feature_combinations_with_mandatory(
    features_dict, X_train_balanced, Y_train_balanced, r, mandatory_features, plot
):
    """
    ...
    for i in tqdm(range(len(feature_combinations))):
        ...
        X_train_transformed = transform_features_no_missing(
        ...
        F1_score = one_run_logistic_regression(
            X_train_transformed,
            Y_train_balanced,
            max_iters=100,
            gamma=0.01,
        ...
        results.append({"features": tuple(feature_combination), "F1_score": F1_score})

    # Sort results by F1_score in descending order to see best combinations at the top
...
def one_run_logistic_regression(
    X, y, max_iters, gamma, reg_norm="", prob_threshold=0.5, batch_size=None, plot=True
):
    ...
    F1_score = F1_score_f(y_te, y_pred, plot=plot)
